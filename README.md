...
<img width="1400" height="560" alt="banner large" src="https://github.com/user-attachments/assets/5a00966a-a93a-4ad6-8e9e-1fac1b6e013f" />
...
| --- | --- |
| scripting | Injects style or script elements into YouTube pages to remove distractions (e.g., hiding recommendations or Shorts). |
| tabs | Applies user-selected preferences across all open YouTube tabs simultaneously. |
| storage | Saves user settings locally so they persist between browser sessions. |
| webNavigation | Detects when a user navigates to blocked YouTube sections (like Shorts) and redirects them appropriately. |
| host permissions | Ensures scripts are injected only on YouTube pages and subdomains relevant to the extension’s functionality. |

### Data Collection and Usage
SileoTube does not collect, store, or transmit any personally identifiable information (PII) or usage analytics.
All settings, preferences, and configurations you make within the extension are stored locally in your browser’s storage using Chrome’s storage API.
This information never leaves your device and is used solely to maintain your preferences between sessions.

### Third-party access
SileoTube does not send any data to external servers or third-party services. There are no analytics tools, advertising scripts, or tracking systems embedded in the extension.

### Policy Compliance
This policy complies with the Chrome Web Store Developer Program Policies, ensuring transparency in data handling and permission use. By using SileoTube, you acknowledge that all data handling is limited strictly to your local browser environment.
